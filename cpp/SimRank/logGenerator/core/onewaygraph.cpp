...
OneWayGraph::~OneWayGraph(){
	...
	if(lm != NULL) {
         ...
         * ToDo: Comment the code only for the test....*/
//		if(lm->hasUpdated()){
//			char filePath[128];
...
void OneWayGraph::mergeLog(){
    if(lm != NULL && lm->isEmpty() == false){
        int* nsrc = new int[maxVertexId + 1];
        int* ndst = new int [maxVertexId];
        int nEdge = 0;
        int shift = 0;
...
//            }
            ...
            int pos=nEdge;
		    lm->getNewNeighbors(vid, ndst+pos, shift);
//            if(sampleId == 1){
//                lm->print(vid);
//		        for(unsigned int i = 0; i < shift; ++i){
//                    printf("ndst=%d\n", ndst[nEdge+i]);
//                }
//                printf("new shift=%d nedge=%d\n", shift, nEdge);
//            }

		    /*clean neighbors */
		    qsort(ndst+nEdge, shift, sizeof(int), cmp1);
		    int pre = -1;
            int ne = 0;
		    for(unsigned int i = 0; i < shift; ++i){
//                if(sampleId == 1){
//                    printf("ndst=%d\n", ndst[nEdge+i]);
//                }
			    if(pre == ndst[nEdge+i] || lm->isChanged(ndst[nEdge+i], vid)) 
                    continue;
                pre = ndst[nEdge+i];
			    ndst[nEdge + ne] = ndst[nEdge+i];
                ne++;
		    }
//            printf("ne=%d\n",ne);
            nsrc[vid] = nEdge;
        ...
        nsrc[maxVertexId] = nEdge;
        lm->clear();
//        printf("old: vid=%d enum=%d; new:vid=%d enum=%d;\n", maxVertexId, edgeNum, maxVertexId, nEdge);
        edgeNum=nEdge;
        delete [] src;
        delete [] dst;
        src=nsrc;
...
void OneWayGraph::setSampleId(int sid){
	sampleId = sid;
}
