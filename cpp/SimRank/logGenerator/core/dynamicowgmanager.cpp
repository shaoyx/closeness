...
void DynamicOwgManager::initialize(){
    for(int sid = 0; sid < sampleGraphNum; sid++)
	    owg[sid]->initialize(maxVertexId, true); //currently only used to test the performance of dynamic. The static graph uses CompressDiskManager();
}
...
/*	if(sampleId == -1) sampleId = sid;
	...
	owg[sid]->addEdge(dst, src);
}

void DynamicOwgManager::save(int sid){
//	owg->setNewSampleId(sampleId);
    for(int i = 0; i < sampleGraphNum; i++){
	    owg[i]->setNewSampleId(i);
        owg[i]->preprocess();
     	owg[i]->save();
    }
}

void DynamicOwgManager::loadIndex(int sid){
    //if(sid != sampleId){
//        sampleId = sid;
		owg[sid]->setSampleId(sid);
		owg[sid]->read();
//        owg[sid]->print();
	//}
}

void DynamicOwgManager::mergeLog(int sid){
    //if(sid != sampleId){
//        sampleId = sid;
   //     printf("Merge %d log\n", sid);
		owg[sid]->setSampleId(sid);
...
void DynamicOwgManager::update(int sid, int src, int dst){
 //   loadIndex(sid);

	owg[sid]->update(src, dst);
}
...
//					for(int zx = 0; zx < size; ++zx){
	 ...
	 //               printf("step=%d: cur=%d\n", step, cur);
					...
					/* enumerate edge here! */
					owg[sid]->expand(cur, cand[(step + 1) & 1]);//, curNbrVid);
