...
inline bool operator< (const vertex_avgdist &la, const vertex_avgdist &lb) {
    if (sabs(la.avgdist - lb.avgdist) > EPSILON) 
...
void stopkrank(int k, vector<int> *person_graph, int num_person, int total_num_person, priority_queue <ValuePair> &ans);
void scyclic_calculation(int qid, set<vertex_centrality>& answer, int k, vector<vertex_degree>& cc_vertices, int cc_size, 
        vector <int> * person_graph, int num_person, int total_num_person);
void supdate_answer(set<vertex_centrality>& answer, int k, double centality, long vid);
void sbfs(int start, int* estimated_distances, vector <int> * person_graph, int total_num_person, int& estimated_radius);

void run(int k, int para) {
    printf("k=%d, para=%d", k, para);
        nmap = (1 << para);
...
vector<ValuePair> Sketch::getResults() {
    return results;
}
...
void stopkrank(int k, vector<int> *person_graph, int num_person, int total_num_person, priority_queue <ValuePair> &ans) {
    ...
    for (int idx = 0; idx < num_person; ++idx) {
        ...
        if(cc_size > 1)
            scyclic_calculation(qid, answers, k, cc_vertices, cc_size, person_graph, num_person, total_num_person);
        else if(cc_size == 1) {
            supdate_answer(answers, k, 0, cc_vertices[0].id);
        }
    }
    delete [] visitedl;

    //printf("answers size: %d\n", answers.size());
 ...
 *
 */
void sprocess(int i, int *dist, int sumDist, set <vertex_centrality> &answer, int *father, int k, vector <int> *vedge, int sumN, int count, bool *prunable, int &total, int level, short& is_changed) {
    level++;
    ...
    for (int y = 0; y < vedge[i].size(); ++y) {
        ...
        sprocess(o, dist, sumDist, answer, father, k, vedge, sumN, count, prunable, total, level, is_changed);
 ...
 * ICDE 2014
 */
void sdelta_pfs(set <vertex_centrality> &answer, int k, vector <vertex_degree> &vp, vector <int> *vedge, int total_num_person, int cc_size, int num_person, short& is_changed) {
    ...
    for (int z = 0; z < vp.size(); ++z) {
        ...
        sprocess(i, dist, sumDist, answer, father, k, vedge, cc_size, num_person, prunable, total, 0, is_changed);
    }
    delete [] vpl;
    delete [] visitedl;
    delete [] v;
 ...
 */
void scyclic_calculation(int qid, set<vertex_centrality>& answers, int k, vector<vertex_degree>& cc_vertices, int cc_size, vector<int> *person_graph, int num_person, int total_num_person) {
    
    int *exact2hop = new int[total_num_person + 1];
    int num_samples = cc_size < 100 ? cc_size : 100;
    ...
    for(int i = 0; i < num_samples; ++i) {
        ...
        int tmp_radius = -1;
        sbfs(start, estimated_distances, person_graph, total_num_person, tmp_radius);
        if(tmp_radius > estimated_maxdepth)
            estimated_maxdepth = tmp_radius;
    }

//    estimated_maxdepth = 6; /**/
    //printf("maxd: %d\n", estimated_maxdepth);
    ...
    sketch[0] = new int*[n + 1];
    sketch[1] = new int*[n + 1];


    for (int iter = 0; iter < cc_size; ++iter) {
        int i = cc_vertices[iter].id;
        sketch[1][i] = new int[nmap + 1];
        sketch[0][i] = new int[nmap + 1];
        memset(sketch[1][i], 0, sizeof(int) * (nmap + 1));
        hashtable[i] = hash1(i); // TODO: mod 10^6 or not?
    }

    int mask = nmap - 1;
    int offset = popcnt(nmap - 1);
    int bigmask = (1 << (MAX_BIT_LENGTH - offset)) - 1;
    ...
    for (int iter = 0; iter < cc_size; ++iter) {
        ...
        if (estimated_maxdepth > 1) estimated_distances[i] -= person_graph[i].size();
#ifdef EXACT2HOP
        if (estimated_maxdepth > 2) estimated_distances[i] -= exact2hop[i] - 1;
#endif
    }

    //printf("max depth: %d\n", estimated_maxdepth);

    for (int d = 2; d < estimated_maxdepth; ++d) {
        //printf("here1\n");
        for (int iter = 0; iter < cc_size; ++iter) {
            //printf("here1.5\n");
            int i = cc_vertices[iter].id;
            int from = (d & 1) ^ 1;
            int to = d & 1;
            memmove(sketch[to][i], sketch[from][i], sizeof(int) * (nmap + 1));
            for (int j = 0; j < person_graph[i].size(); ++j)
                for (int t = 0; t < nmap; ++t)
                    sketch[to][i][t] |= sketch[from][person_graph[i][j]][t];

            int s = 0;
            for (int j = 0; j < nmap; ++j) {
                unsigned int x = sketch[to][i][j];
                int y = x & bigmask;
                s += popcnt((~y & (y + 1)) - 1);
            }
            //printf("here2\n");
            int en = pow((double)2, (double)s / nmap) * nmap;

            //if (qid == 0) printf("%d %d %d %.lf %d\n", d, i, person_graph[i].size(), (double)en / phi, cc_size);
    ...
    /*
     * precise calculation of the candidate set
     */
#ifdef MIXFRAMEWORK
    int topke = 50 * k; // choose top 100*k of estimated closeness
    ...
    short is_changed = 0;
    sdelta_pfs(answers, k, vp, person_graph, total_num_person, cc_size, num_person, is_changed);
#endif

    /*
     * use the estimated result directly
     */
#ifndef MIXFRAMEWORK
    //printf("k: %d\n", k);
    for(int i = 0; i < cc_size; ++i) {
        ...
        double centrality = (double)(cc_size - 1) * (cc_size - 1) / (avgdist + (cc_size - 1) * estimated_maxdepth) / (num_person - 1);
        //printf("%d %d %d %d %lf %lf\n", k, vid, cc_size, num_person, avgdist, centrality);
        ...
        if (answers.size() > k) {
            printf("%d %d %d %d %lf %lf\n", k, vid, cc_size, num_person, avgdist, centrality);
            printf("%d %lf\n", (*answers.rbegin()).id, (*answers.rbegin()).closeness);
...
void supdate_answer(set<vertex_centrality>& answer, int k, double centrality, long vid) {
