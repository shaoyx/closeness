...
@GraphAnalyticTool(
name = "SimRankGSQvCrossV",
description = "Compute SimRank based on graph sampling, and use random walks across R samples."
)
public class SimRankGSCrossV 
implements GenericGraphTool{
	...
	public void topksimrank(){
		sampledEdges = new int[sampleNum][vertexMap.size()];
		...
		for(int i = 0; i < sampleNum; i++){
			for(int vid = 0; vid < vertexMap.size(); vid++){
				...
				if(nb == null || nb.size() == 0){
					sampledEdges[i][vid] = -1;
				}
				else{
					sampledEdges[i][vid] = nb.get((int)(getRandomNextInt() % nb.size()));
				}
			}
		}
...
//		System.out.println("(301, " + String.format("%.9f", simrankValues[vertexMap.get(301)] / sampleNum)+")");
		while(pq.isEmpty() == false){
			...
			System.out.println(cnt+" ==> ("+ rvertexMap.get(srp.getVertex())+", " + String.format("%.9f", srp.getSimRank() / sampleNum / sampleNum)+")");
			cnt--;
		}
	}
	
	private void computePairWiseSimRank(int idx) {
		/* 1. calculate a random walk for the queryVertex. */
		visitedTimestamp.clear();
		int curVertex = qv;
		for(int s = 1; s <= MAX_STEPS; s++){
			int next = sampledEdges[idx][curVertex];
			if(next == -1){
				break;
			}
...
//				System.out.println("arrayLengh="+arrayLen+" tail="+tail);
				...
				while(step <= lastUniqVertex + MAX_STEPS){
					int next = sampledEdges[iter][path[((tail - 1) + arrayLen) % arrayLen]];
					if(next == -1) break;
