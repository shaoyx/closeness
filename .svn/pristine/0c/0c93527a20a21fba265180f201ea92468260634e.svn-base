...
int LogManager::MAX_LOG_RECORD_NUM = 1010;
...
void LogManager::getNewNeighbors(int vid, int* nb, int& offset){
	int next = startIdx[vid];
//    if(isPrint) printf("vid=%d: start=%d: ", vid, next);
	while(next != -1){
		nb[offset] = neighborList[next];
        offset++;
 //       if(isPrint) printf("(offset=%d, %d,%d)", offset, neighborList[next], pointList[next]);
		next = pointList[next];
	}
  //  if(isPrint) printf("\n");
}
...
void LogManager::read(char* filepath){
    ...
    if(fp == NULL){
     //   printf("Index log doesn't exist.\n");
        return;
    }
	hasNewLog = false;
...
void LogManager::print(int vid){
	int next = startIdx[vid];
    printf("vid=%d start=%d\n", vid, next);
	while(next != -1){
        printf("nb=%d, next=%d\n",neighborList[next], pointList[next]);
        next = pointList[next];
	}
}
